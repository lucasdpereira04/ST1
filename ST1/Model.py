...
class BoatModel:
    ...
    def handle_missing_values(self):
        ...
        quantitative_columns = self.data.select_dtypes(include=['int64', 'float64']).columns
        categorical_columns = self.data.select_dtypes(include=['object']).columns
        # Assign missing values for continuous variables with median
        for column in quantitative_columns:
            median = self.data[column].median()
            self.data[column] = self.data[column].fillna(median)
        # Assign missing values for categorical variables with mode
        for column in categorical_columns:
    ...
    def feature_selection(self):
        ...
        # Creates a scatter plot for continuous vs continuous
        quantitative_columns = self.data.select_dtypes(include=['int64', 'float64']).columns
        for column in quantitative_columns:
